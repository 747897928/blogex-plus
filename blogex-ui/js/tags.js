new Vue({
    ...
    mounted() {
        let tagId = getParams("tagId");
        if (tagId !== "") {
            let _that = this;
            tagId = parseInt(tagId, 10);
            this.tagItem.id = tagId;
            this.articlePageQuery.tagId = tagId;
            axiosGetFunction('/api/tag/openApi/detail/'+tagId, null, false).then((response) => {
                if (response){
                    this.tagItem = response;
                }
            });
