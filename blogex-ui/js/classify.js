const vueObject = new Vue({
    ...
    mounted() {
        let classifyId = getParams("id");
        if (classifyId !== "") {
            let _that = this;
            classifyId = parseInt(classifyId, 10);
            this.classifyItem.id = classifyId;
            this.articlePageQuery.classifyId = classifyId;
            axiosGetFunction('/api/classify/openApi/detail/'+classifyId, null, false).then((response) => {
               if (response){
                   this.classifyItem = response;
               }
            });
